...
class MajorController extends Controller
{
    ...
    {
        // 1. Tự động kiểm tra: Nếu DB có ít hơn 10 ngành, tiến hành chèn dữ liệu từ sql.sql
        if (Major::count() < 10) {
            ...
            if (File::exists($sqlPath)) {
                ...
                // Lấy tất cả các câu lệnh INSERT INTO
                preg_match_all('/INSERT INTO\s+["`]?majors["`]?\s*\(([^)]+)\)\s*VALUES\s*(.+?);/is', $content, $matches, PREG_SET_ORDER);

                foreach ($matches as $match) {
                    $columnsRaw = $match[1];
                    $valuesRaw  = $match[2];

                    // Làm sạch tên cột (bỏ dấu ` của MySQL)
                    $columns = array_map(function ($col) {
                        return trim(str_replace(['`', '"'], '', $col));
                    }, explode(',', $columnsRaw));

                    // Phân tách từng dòng VALUES trong câu lệnh INSERT
                    preg_match_all('/\((?>[^()]+|(?R))*\)/x', $valuesRaw, $valueRows);

                    foreach ($valueRows[0] as $row) {
                        $rowContent = trim($row, '()');
                        // Tách các giá trị bởi dấu phẩy (bỏ qua dấu phẩy nằm trong chuỗi)
                        $vals = str_getcsv($rowContent, ',', "'");
                        ...
                        foreach ($columns as $index => $colName) {
                            ...
                            if ($val === 'NULL' || $val === 'null' || $val === null) {
                                ...
                                // Xóa các ký tự escape dư thừa
                                $data[$colName] = str_replace(["\\'", '\\"'], ["'", '"'], $val);
                            }
                        }

                        // Chèn từng bản ghi vào DB, bỏ qua nếu trùng ID hoặc Code
                        if (isset($data['id'])) {
                            Major::updateOrCreate(['id' => $data['id']], $data);
                        } else {
                            Major::create($data);
                        }
                    }
                ...
                // Cập nhật lại Sequence ID cho PostgreSQL
                try {
                    $maxId = Major::max('id') ?? 0;
                    if ($maxId > 0) {
                        DB::statement("SELECT setval('majors_id_seq', {$maxId});");
                    }
                    Artisan::call('storage:link');
                } catch (\Exception $e) {
                    // Bỏ qua lỗi tạo link nếu đã tạo rồi
                }
            }
        }

        // 2. Trả dữ liệu ngành nghề ra Frontend
        $items = Major::query()
            ->where(function ($sub) {
                $sub->whereNull('status')
                    ->orWhere('status', 'active');
