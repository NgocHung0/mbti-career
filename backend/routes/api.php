...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::get('/import-full-sql', function () {
    $sqlPath = database_path('sql.sql'); 

    if (!File::exists($sqlPath)) {
        return response()->json([
            'error' => 'Khong tim thay file database/sql.sql! Hay dam bao ban da coppy file sql.sql vao thu muc database/ cua du an.'
        ], 404);
    }
    ...
    try {
        ...
        return response()->json(['message' => 'DA IMPORT THANH CONG 100 NGANH NGHE VA TAO LINK ANH!']);
    } catch (\Exception $e) {
